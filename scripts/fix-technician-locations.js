...
const GeocodingService = require('../backend/src/infrastructure/external/GeocodingService');
      ...
      try {
        ...
        if (typeof addressObj === 'string') {
          try {
            ...
            console.warn(
              `Address is a string but not JSON for tech ${tech.id}: ${addressObj}`
            );
            continue;
          }
        }
        ...
        if (coords) {
          ...
          console.log(
            `Updated technician ${tech.id} with location: ${coords.latitude}, ${coords.longitude}`
          );
        } else {
