...
const { getPool } = require('../../infrastructure/database/pgClient');
...
function detectVia(req) {
  const headers = (req && req.headers) || {};
  const hasCloudflareHeader = Object.keys(headers).some((h) =>
    h.toLowerCase().startsWith('cf-')
  );
  const host = headers['host'] || '';
  if (hasCloudflareHeader || /\.trycloudflare\.com$/i.test(host))
    return 'cloudflare';
  const forwarded = req.get && req.get('X-Forwarded-For');
...
function logLogin(userInfo, req) {
  ...
  console.log(
    `[AUDIT] Login realizado - Usuário: ${userInfo.email} | IP: ${maskedIP}`
      .green
  );
  try {
    const pool = getPool();
    pool.query(
      ...
      [
        userInfo.id || null,
        'login',
        JSON.stringify({ ip: maskedIP, ua: userAgent }),
      ]
    );
  } catch {}
}
...
function logFailedLogin(email, req, reason = 'Credenciais inválidas') {
  ...
  // Log no console também
  console.log(
    `[AUDIT] Tentativa de login falhada - Email: ${email} | IP: ${maskedIP} | Motivo: ${reason}`
      .red
  );
  try {
    const pool = getPool();
    pool.query(
      ...
      [
        null,
        'login_failed',
        JSON.stringify({ email, ip: maskedIP, ua: userAgent, reason }),
      ]
    );
  } catch {}
}
...
function logLogout(userInfo, req) {
  ...
  console.log(
    `[AUDIT] Logout realizado - Usuário: ${userInfo.email} | IP: ${maskedIP}`
      .yellow
  );
  try {
    const pool = getPool();
    pool.query(
      'INSERT INTO analytics_events (user_id, event, metadata) VALUES ($1,$2,$3)',
      [userInfo.id || null, 'logout', JSON.stringify({ ip: maskedIP })]
    );
  } catch {}
}
